...
export default function HomePage() {
  ...
  const deposit = async () => {
    if (atm && amount > 0) {
      ...
      // Check if the deposited amount is greater than 100 ETH
      if (amount > 100) {
        const rewardTx = await atm.deposit(ethers.utils.parseEther("10.0"));
        await rewardTx.wait();
        alert("You have been rewarded with an additional 10 ETH!");
        getBalance();
      }
    }
  };
      ...
      <div>
        ...
        {!isVerified ? (
          ...
          <div className="action-selection">
            ...
            <button className="btn" onClick={() => setAction("buyProduct")}>Buy Product</button>
          </div>
        ) : action === "deposit" ? (
          <div>
          ...
          <div>
            <button className="btn" onClick={() => buyProduct(1)}>Buy Product for 1 ETH</button>
            <button className="btn" onClick={() => buyProduct(2)}>Buy Product for 2 ETH</button>
            <br/><br/><button className="btn" onClick={() => setAction(undefined)}>Back</button>
          </div>
        )}
      </div>
  ...
  return (
    <main className="container">
      <header className="header">
        <h1>Digital Decentralized Currency Exchange</h1>
      </header>
